...
class Redis_Store():
    ...
    def create_user_session(self, user_id: int, client_ip: str) -> str:
        ...
        self.redis_client.hset(f"user_session:{session_id}", mapping={
            "account_id": user_id,
            "client_ip": client_ip,
        ...
        self.redis_client.lpush(f"user_sessions:{user_id}", session_id)

        return session_id
            
    def clear_user_sessions(self, user_id: int, client_ip: str) -> None:
        existing_user_sessions = self.redis_client.lrange(f"user_sessions:{user_id}", 0, -1)    
        existing_ip_sessions = self.redis_client.lrange(f"user_sessions:{client_ip}", 0, -1)
        
        existing_sessions = set(existing_user_sessions + existing_ip_sessions) # type: ignore
        for session in existing_sessions:
            self.redis_client.delete(f"user_session:{session}")
            
        self.redis_client.delete(f"user_sessions:{user_id}")
        self.redis_client.delete(f"user_sessions:{client_ip}")
        
    def get_game_data(self, session_jwt: str) -> Game_Data:
        result = self.redis_client.get(f"game_data:{session_jwt}")
